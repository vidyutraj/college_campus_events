...
export default function Events({ myEventsOnly = false }: EventsProps) {
  ...
  const approvedEvents = events.filter(
    (event) => event.is_approved && event.status === "published"
  );

  // Map events to FullCalendar format
  const calendarEvents = approvedEvents.map((e) => ({
    id: e.id.toString(),
    title: e.title,
    ...
    <div className="max-w-7xl mx-auto px-5 py-8">
              ...
              <div className="space-y-2">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    onChange={(e) =>
                      handleFilterChange("hasFreeFood", e.target.checked)
                    }
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    onChange={(e) =>
                      handleFilterChange("hasFreeSwag", e.target.checked)
                    }
                    className="w-4 h-4 text-primary focus:ring-primary border-gray-300 rounded"
                  />
      ...
      {hasActiveFilters() && !showFilters && (
        <div className="bg-blue-50 border border-blue-200 rounded p-4 mb-6 text-blue-800">
          Filters active: {approvedEvents.length} event
          {approvedEvents.length !== 1 ? "s" : ""} found
        </div>
      )}
          ...
          <button
            ...
            className={`w-1/2 px-4 py-2 btn border-y border-l rounded-l-lg ${
              view === "list" ? "bg-secondary font-medium border-secondary-dark border-r text-background" : "border-foreground/50"
            }`}
          >
            <LuList /> List View
          </button>
          <button
            ...
            className={`w-1/2 px-4 py-2 btn border-y border-r rounded-r-lg ${
              view === "calendar" ? "bg-secondary font-medium border-secondary-dark border-l text-background" : "border-foreground/50"
            }`}
          >
      ...
      {view === "list" ? (
        ...
        <div className="bg-white rounded-lg p-4 shadow">
          <FullCalendar
            ...
            eventColor='#9657e8'
            eventClick={(info) => setSelectedEvent(info.event.extendedProps as Event)} 
          />
        </div>
      )}
