...
const MODALITY_OPTIONS: Array<{ value: EventFormValues['modality']; label: string }> = [
  { value: 'in-person', label: 'In-Person' },
  { value: 'online', label: 'Online' },
  { value: 'hybrid', label: 'Hybrid' },
];

const STATUS_OPTIONS: Array<{ value: EventFormValues['status']; label: string }> = [
...
function AdminEvents() {
  ...
  const fetchEvents = useCallback(async () => {
    const response = await axiosInstance.get(ADMIN_EVENTS_URL);
    setEvents(response.data.results || response.data);
  }, []);

  const fetchLookups = useCallback(async () => {
    const [categoriesResponse, organizationsResponse] = await Promise.all([
      axiosInstance.get(CATEGORIES_URL),
      axiosInstance.get(ORGANIZATIONS_URL),
    ]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        setPageError('Failed to load admin data. Please try again later.');
      } finally {
        setPageLoading(false);
      }
    };
  ...
  const handleEdit = (event: Event) => {
    ...
    setFormValues({
      ...
      hostOrganizationId: event.host_organization ? String(event.host_organization.id) : '',
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const payload = {
      ...
      end_datetime: formValues.endDatetime ? toISODateTime(formValues.endDatetime) : null,
      ...
      category_id: formValues.categoryId ? Number(formValues.categoryId) : null,
      subcategory: formValues.subcategory,
      host_organization_id: formValues.hostOrganizationId ? Number(formValues.hostOrganizationId) : null,
    ...
    if (!payload.start_datetime) {
      setFormError('Start date could not be parsed. Please use the selector to choose a date.');
      return;
    }
    ...
    try {
      if (editingEventId) {
        await axiosInstance.put(`${ADMIN_EVENTS_URL}${editingEventId}/`, payload);
        setStatusMessage('Event updated successfully.');
      } else {
        await axiosInstance.post(ADMIN_EVENTS_URL, payload);
      ...
      setFormError(apiError || 'Unable to save the event. Please review the details and try again.');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (eventToDelete: Event) => {
    if (!window.confirm(`Delete "${eventToDelete.title}"? This cannot be undone.`)) {
      return;
    }
    ...
    setFormError(null);
    try {
      await axiosInstance.delete(`${ADMIN_EVENTS_URL}${eventToDelete.id}/`);
      ...
      const apiError =
        ...
        err.response?.data?.message ||
        err.response?.data?.non_field_errors?.[0];
      setFormError(apiError || 'Failed to delete the event. Please try again.');
    } finally {
      setDeletingId(null);
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-5 py-10 space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-800">Event Administration</h1>
          <p className="text-gray-600">Manage all events, including drafts and unpublished items.</p>
        </div>
        <button
          type="button"
          onClick={resetForm}
              ...
              {events.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-4 py-6 text-center text-gray-500">
                    No events found. Use the form to create the first event.
                  </td>
                </tr>
              ) : (
                events.map(event => (
                  <tr
                    key={event.id}
                    className={editingEventId === event.id ? 'bg-blue-50/50' : undefined}
                  >
        ...
        <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-6 space-y-4">
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="modality">
                  Modality
                </label>
                <select
                  id="modality"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="hostOrganizationId">
                  Host Organization
                </label>
                <select
                  id="hostOrganizationId"
                  value={formValues.hostOrganizationId}
                  onChange={e => handleChange('hostOrganizationId', e.target.value)}
                  className="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary"
                >
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="subcategory">
                Subcategory
              </label>
              <input
                id="subcategory"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="employersInAttendance">
                Employers / Companies in Attendance
              </label>
              <textarea
                id="employersInAttendance"
                value={formValues.employersInAttendance}
                onChange={e => handleChange('employersInAttendance', e.target.value)}
                className="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary"
                rows={2}
                placeholder="Optional"
              ></textarea>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="otherPerks">
                Other Perks
              </label>
              <textarea
                id="otherPerks"
            ...
            <div className="flex items-center gap-4">
              <label className="flex items-center gap-2 cursor-pointer text-sm text-gray-700">
                <input
                  ...
                  onChange={e => handleChange('hasFreeFood', e.target.checked)}
              ...
              <label className="flex items-center gap-2 cursor-pointer text-sm text-gray-700">
                <input
                  ...
                  onChange={e => handleChange('hasFreeSwag', e.target.checked)}
                  className="w-4 h-4 text-primary border-gray-300 rounded focus:ring-primary"
                />
                Free Swag
              </label>
